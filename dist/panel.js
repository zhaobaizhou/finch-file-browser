...
`;
    ...
    return {
      ...
      panel: { setTitle: async () => void 0, setIcon: async () => void 0 }
    };
  }
  var bridge = window.finch ?? createDemoBridge();
  var S = {
    root: "",
    rootLabel: "",
    locale: "zh-CN",
    ...
    // expanded directory rels
    pendingDirs: /* @__PURE__ */ new Set(),
  ...
  var toastTimer = null;
  function toast(text2) {
    ui.toast.textContent = text2;
  ...
  var ICON_MD = '<svg viewBox="0 0 24 24"><path d="M14 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V8Z"/><path d="M14 3v5h5"/><path d="M8 17v-4l1.6 2L11 13v4M14.5 13v4M13 15.5l1.5 1.5 1.5-1.5"/></svg>';
  ...
  function renderFoot() {
    const total = [...S.dirs.values()].reduce((sum, list2) => sum + list2.length, 0);
    ui.foot.innerHTML = `<span><span class="legend-dot" style="background:var(--fb-accent)"></span>\u672C\u4F1A\u8BDD\u6539\u52A8 ${S.changed.size}</span><span><span class="legend-dot" style="background:var(--fb-warning)"></span>\u5BF9\u8BDD\u6D89\u53CA ${S.touched.size}</span><span class="grow"></span><span>\u5DF2\u5217\u51FA ${total}</span>`;
  }
  ...
  var EMPTY_MARKUP = '<div class="empty-title">\u9009\u62E9\u4E00\u4E2A\u6587\u4EF6\u5F00\u59CB</div><div class="empty-sub">\u53F3\u4FA7\u662F\u8FD9\u4E2A\u5BF9\u8BDD\u7ED1\u5B9A\u6587\u4EF6\u5939\u91CC\u7684\u5168\u90E8\u6587\u4EF6</div>';
  function resetEmpty() {
  ...
  function renderPreview(file) {
    if (file.flavor === "markdown") {
      ...
      ui.preview.innerHTML = purify.sanitize(html3, { ADD_ATTR: ["target"] });
      ui.preview.querySelectorAll("pre code").forEach((block2) => {
        try {
          common_default.highlightElement(block2);
        } catch {
        }
      });
    } else {
      ...
      if (file.flavor === "code") {
        try {
          code.innerHTML = common_default.highlightAuto(file.content).value;
  ...
  function showBinaryCard(file) {
    ...
    meta.className = "meta";
    meta.textContent = `${file.kind === "too-large" ? "\u6587\u4EF6\u8FC7\u5927" : "\u4E8C\u8FDB\u5236\u6587\u4EF6"} \xB7 ${fmtSize(file.size)}`;
  ...
  function renderSearchResults(message) {
    ...
    head.textContent = `${S.searchResults.length} \u4E2A\u5339\u914D \xB7 \u201C${message.query}\u201D`;
