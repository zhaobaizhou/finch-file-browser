...
- **自动保存模式**：打开后隐藏「保存」按钮，停止输入约 0.9 秒自动写回磁盘，顶部用一个状态点表示「未保存 / 保存中 / 已保存」。切文件前会先落盘，不再弹「放弃修改吗」。当前文件在磁盘上被人改过时，自动保存会**暂停**并提示你选择覆盖或重新载入，不会悄悄覆盖别人的改动
- 保存前校验磁盘 mtime：如果文件在你编辑期间被 Agent 改过，会弹提示，让你选**以我的版本覆盖**或**先重新载入**
- **回滚上次保存**：每次保存前会把上一版内容存进小程序私有目录，可以原地回滚到保存前的状态（一次性，回滚后按钮自动变灰）。不想占这份空间可以在设置里关掉
- 手动模式下切换文件时若有未保存改动会先确认
...
| `autoSave` | 关 | 输入时自动保存，隐藏「保存」按钮 |
| `keepBackups` | 开 | 每次保存前留一份可回滚的副本 |
