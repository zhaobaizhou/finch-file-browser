...
**撤销本次编辑**（在 `⋯` 菜单里）：把文件恢复到**这个会话第一次打开它时**的内容。它不是"回滚上次保存"—— 自动保存下"上次保存"只有一秒前，没有意义。而且它是**双向的**：按下之后菜单会变成「恢复我的编辑 +N 行」，再按一次就把你的编辑换回来，永远不会单向掉进一个你事先看不见的版本里。按钮上直接标出会增减多少行。
...
菜单下方还有「这个文件」一组：**撤销本次编辑**（双向，含行数增减提示）。
...
| `keepBackups` | 开 | 保留「打开时」的快照，用于「撤销本次编辑」 |
